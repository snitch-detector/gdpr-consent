...
DEVICE_PII_DICT = {
    ...
    'emulator-5554': {
        'EMAIL': 'j.drescher@tu-bs.de',
        'GSF': 'this is an emulator'},
    '0616064d1eeb0c05': {
        'EMAIL': 'j.drescher@tu-bs.de',
        'GSF': 'this is my old s6'}
}
...
class DEVICE:
    ...
    def get_advertising_id(self):
        ...
        adb_object = ADB(Device(device_id))
        try:
            output = adb_object.run_cmd(
                f'shell su 0 grep adid_key /data/data/com.google.android.gms/shared_prefs/adid_settings.xml')
            for line in output.split('\n'):
                root = ET.fromstring(line.strip())
                if root.text is not None:
                    print(f"AAID: {root.text}")
                    adid_list.append(root.text)
            return adid_list
        except Exception as e:
            print(
                "AAID not found, this device probably doesn't have Play Services installed")
            return ["This is a dummy value for the exception case"]
    ...
    def get_service_call(self, service_call_code):
        ...
        adb_object = ADB(Device(device_id))
        output = adb_object.run_cmd(
            f'shell su 0 service call iphonesubinfo {service_call_code}')
        value_list = []
        for line in output.split('\n'):
            if '\'' not in line:
                continue
    ...
    def get_installed_packages(self):
        ...
        package_list = []
        adb_object = ADB(Device(device_id))
        output = adb_object.run_cmd('shell su 0 cmd package list packages')
        for line in output.split('\n'):
    ...
    def get_contacts(self):
        ...
        adb_object = ADB(Device(device_id))
        output = adb_object.run_cmd(
            'shell su 0 content query --uri content://contacts/phones/')
        for line in output.split('\n'):
            line_list = line.split(', ')
            contact_dict = dict()
...
def get_path(apk, home_dir):
    ...
    file_path = os.path.join(
        home_dir, sha1String[0], sha1String[1], sha1String[2], sha1String[3], apk)
...
def detect_pii_parameter(log_parameter, hashed_pii_dict):
    ...
    try:
        base64decoded = base64.b64decode(
            log_parameter.parameter_value.encode())
...
def init_pii_dict(device_serial):
    ...
    DEVICE_PII_DICT[device_serial]['AID'] = device_object.get_advertising_id()[
        0]
    ...
    for key in GPS_DICT:
        for value in GPS_DICT[key]:
            ...
            for hashed_value in hashed_value_list:
                ...
                if hashed_value != '':
                    hashed_pii_dict[device_serial][key].add(hashed_value)
                    hashed_pii_dict[device_serial][key].add(
                        hashed_value.lower())
                    hashed_pii_dict[device_serial][key].add(
                        hashed_value.upper())
    ...
    with open(file_path, 'rb') as fp:
        ...
        for flow in reader.stream():
            ...
            try:
                ...
                traffic_log_list.append(TRAFFIC_LOG(device_serial, package_name, file, json.dumps(
                    request_header_dict), flow.request.host, json.dumps(host_query_dict), flow.request.raw_content, json.dumps(request_dict)))
                ...
                try:
                    ...
                    try:
                        ...
                        traffic_log_list.append(TRAFFIC_LOG(device_serial, package_name, file, json.dumps(
                            request_header_dict), flow.request.host, json.dumps(host_query_dict), flow.request.raw_content, json.dumps(parsed_data)))
                    except Exception as e:
                        raise e1
                except Exception as e:
                    ...
                    traffic_log_list.append(TRAFFIC_LOG(device_serial, package_name, file, json.dumps(
                        request_header_dict), flow.request.host, json.dumps(host_query_dict), flow.request.raw_content, json.dumps("")))
                    pass
...
def recursive_insert(traffic_log_parameters, package_name, device_serial, file_name, host_name, data, prefix=''):
    ...
    if isinstance(data, dict):
        for key, value in data.items():
            ...
            recursive_insert(traffic_log_parameters, package_name,
                             device_serial, file_name, host_name, value, db_key)
        return

    if isinstance(data, list):
        for item in data:
            recursive_insert(traffic_log_parameters, package_name,
                             device_serial, file_name, host_name, item, prefix)
        return
    ...
    if isinstance(data, str):
        ...
        if is_json_dict:
            recursive_insert(traffic_log_parameters, package_name,
                             device_serial, file_name, host_name, data, prefix)
            return
        ...
        if host_name == 'www.facebook.com':
            if data.startswith('{"context"') or data.startswith('{"request"'):
                ...
                recursive_insert(traffic_log_parameters, package_name,
                                 device_serial, file_name, host_name, eval(data), prefix)
            else:
                traffic_log_parameters.append(TRAFFIC_LOG_PARAMETER(
                    package_name, device_serial, file_name, host_name, prefix, str(data)))
            return

        if 'rayjump.com' in host_name and prefix == 'data':
            parsed = parse_qs(data)
            recursive_insert(traffic_log_parameters, package_name,
                             device_serial, file_name, host_name, parsed, prefix)
            return
        ...
        if 'smartadserver.com' in host_name and (prefix == ' name' or prefix == 'name'):
            ...
            for item in arrs:
                ...
                if item.startswith('{'):
                    recursive_insert(traffic_log_parameters, package_name,
                                     device_serial, file_name, host_name, json.loads(item), prefix)
                if item.startswith('--'):
                    traffic_log_parameters.append(TRAFFIC_LOG_PARAMETER(
                        package_name, device_serial, file_name, host_name, prefix, str(item.strip('--'))))
            return
        ...
        if 'taboola.com' in host_name and (prefix == 'fil'):
            ...
            if len(arrs) == 2:
                ...
                recursive_insert(traffic_log_parameters, package_name,
                                 device_serial, file_name, host_name, item_1_dict, prefix)

                item_2 = '{' + arrs[1] + '"}'
                recursive_insert(traffic_log_parameters, package_name, device_serial,
                                 file_name, host_name, json.loads(item_2), prefix)
            return
        if 'taboola.com' in host_name and (prefix == 'events'):
            item = data + '"}]'
            item_dict = eval(item)[0]
            recursive_insert(traffic_log_parameters, package_name,
                             device_serial, file_name, host_name, item_dict, prefix)
            return
    ...
    try:
        ...
        try:
            if '\\x' in decoded_request_body:
                decoded_request_body = zlib.decompress(
                    request_body, 16 + zlib.MAX_WBITS)
        except Exception as e:
            pass
        ...
        try:
            ...
            recursive_insert(traffic_log_parameters, package_name,
                             device_serial, file_name, host_name, request_query_dict)
        except Exception as e:
        ...
        if is_dict_type:
            try:
                recursive_insert(traffic_log_parameters, package_name,
                                 device_serial, file_name, host_name, json_dict)
            except Exception as e:
                print(e)
...
def main():
    ap = argparse.ArgumentParser(
        description='Detecting personal data in the network traffic by String-Matching Device-Bound Data')
    ...
    for package_name in apk_file_dict:
        ...
        for file in os.listdir(dir_path):
            ...
            traffic_logs.extend(read_request_flow(
                file_path, args.serial, package_name, file))
    ...
    for log in traffic_logs:
        ...
        for log_parameter in traffic_parameters:
            requested_pii_set = detect_pii_parameter(
                log_parameter, hashed_pii_dict)
